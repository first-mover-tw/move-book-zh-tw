...
SYSTEM_PROMPT = (
    ...
    # 本段只管**內文**。標題另有上面那條「中文譯文 (英文原文)」慣例，它對含
    # 強調的標題必然讓強調變 2–3 倍（`### Trick #1 - _any_ Condition`），且括號
    # 內的英文原文要一字不差（gate 9 verbatim）——不寫清楚範圍，兩條硬規則會
    # 互相矛盾，模型只能自己挑一條。gate 10 本身也排除標題。
    "內文（標題除外）的強調（`*斜體*`、`**粗體**`）必須原位保留，"
    "數量、型別、順序與原文完全相同：\n"
    "- 不可把 `*斜體*` 譯成 `**粗體**`、也不可用「」或引號取代強調。\n"
    "- 內文中文一律用 `*文字*`，不要用 `_文字_` —— CJK 相鄰時 `_` 渲染不出強調。"
    "標題結尾括號內的英文原文不適用本條，一律一字不差照抄。\n"
    "- 不要自行增加原文沒有的強調。\n"
    "標題翻譯規則（違反任何一條整份重來）：\n"
