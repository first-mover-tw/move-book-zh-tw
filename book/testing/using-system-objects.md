---
description: "在 Move 測試中使用系統物件：建立並操作 Clock、Random 和 DenyList，用於測試時間、隨機數和拒絕清單。"
---

# 在測試中建立和使用系統物件 (Creating and Using System Objects in Tests)

某些測試需要系統物件，例如 `Clock`、`Random` 或 `DenyList`。這些物件在網絡上有
[固定位址](./../appendix/reserved-addresses.md)，並在創世時建立。在測試中，它們預設不存在，所以 Sui Framework 提供 `#[test_only]`
函式來建立和操作它們。

## Clock（時鐘）

[`Clock`](./../programmability/epoch-and-time.md#clock) 提供當前網絡時間戳。
使用 `clock::create_for_testing` 建立一個，並透過測試專用函式操作時間：
...
若要在測試情境中共享 `Clock`，呼叫 `share_for_testing`：
...
`Random` 物件提供鏈上隨機性。在測試中，完整的 `Random` 共享物件只能
在 [測試情境](./test-scenario.md) 內透過 `random::create_for_testing` 建立。不過，
較佳做法是將程式碼結構化，使核心邏輯接受 `RandomGenerator`
參數 - 這樣可以在單元測試中直接用 `random::new_generator_for_testing()` 建立產生器，
完全略過 `Random` 物件。這更容易使用，因為 `Random` 需要 `entry` 函式（無法傳回非可銷毀值），
導致難以對結果進行斷言。

```move
use sui::random::{Self, Random, RandomGenerator};

// 若要使用 Random，函式必須有 `entry` 修飾符，因此無法傳回
// 值，也不容易測試。
...
對於接受完整 `Random` 共享物件的進入點（唯一的方式是以參考 `&Random` 方式接受），
使用 [測試情境](./test-scenario.md)：
...
`DenyList` 由受管制的代幣用來阻止特定位址。用 `new_for_testing` 建立本地實例，
或用 `create_for_testing` 建立共享實例：
...
## Coin 和 Balance（代幣和餘額）

用代幣進行測試時，使用 `coin::mint_for_testing` 和 `balance::create_for_testing`：
...
## 一次建立所有系統物件 (Create All System Objects at Once)

當使用 [測試情境](./test-scenario.md) 時，可以用 `create_system_objects` 一次建立所有系統物件。
這會建立並共享 `Clock`、`Random` 和 `DenyList`：
...
fun test_with_all_system_objects() {
    ...
    // 將 Clock、Random 和 DenyList 建立為共享物件
...
> 在測試中建立的系統物件不會有它們在活躍網絡上的固定位址。
> 使用 `take_shared<T>()` 按類型而非按 ID 來存取它們。

若要按 ID 取得特定共享物件，使用 `take_shared_by_id`：
