...
- 內部型別 (Internal Type) - 模組內部定義的型別。此型別的欄位無法從模組外部存取，且在只有「key」能力的情況下，無法用於 `public_*` 轉移函式。

## 能力 (Abilities) {#abilities}

- `key` - 允許結構體在儲存中作為鍵使用的能力。在 Sui 上，`key` 能力會標記物件，並要求第一個欄位為 `id: UID`。
- `store` - 允許結構體儲存在其他物件內部的能力。此能力放寬了對內部結構體的限制，允許 `public_*` 轉移函式接受它們作為引數。它還允許物件作為動態欄位儲存。
