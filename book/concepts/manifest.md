---
description: Move.toml 套件清單 (Package Manifest)：套件 (package) 中繼資料 (metadata)、依賴 (dependencies)、具名 (named) 地址 (addresses) 與依賴 (dependency) 覆寫 (overrides) 說明。
title: 套件清單 (Package Manifest)
keywords:
  - Move
  - Sui
  - Move tutorial
...
answer: 'The Move.toml package manifest: package metadata, dependencies, named addresses, and dependency overrides explained.'
goal:
  description: 'Reader understands the Move.toml package manifest: package metadata, dependencies, named addresses, and dependency overrides explained'
  requires:
    - has_frontmatter:
        - title
        - description
...
`Move.toml` 是描述[套件 (package)](./packages) 及其依賴項的清單檔案。它以 [TOML](https://toml.io/en/) 格式撰寫，包含多個區段，其中最重要的是 `[package]`、`[dependencies]` 和 `[addresses]`。
...
`[package]` 區段用於描述套件。此區段中的任何欄位皆不會發佈至鏈上，但會用於工具和發佈管理；它們也指定了編譯器的 Move 版本。
...
`[dependencies]` 區段用於指定專案的依賴項。每個依賴項都以鍵值對的形式指定，其中鍵是依賴項的名稱，值是依賴項規格。依賴項規格可以是 Git 儲存庫 URL 或本地目錄的路徑。
...
有時候依賴項之間會出現同一個套件的版本衝突。例如，如果你有兩個依賴項使用不同版本的 Example 套件，你可以在 `[dependencies]` 區段中覆寫該依賴項。為此，請將 `override` 欄位新增至依賴項。將會使用 `[dependencies]` 區段中指定的依賴項版本，而非依賴項本身指定的版本。
...
TOML 格式支援兩種表格樣式：內嵌 (inline) 和多行 (multiline)。上述範例皆使用內嵌樣式，但也可以使用多行樣式。你不會想將其用於 `[package]` 區段，但對於依賴項可能很有用。
