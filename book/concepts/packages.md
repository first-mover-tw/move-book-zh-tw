---
description: 理解 Move 套件 (Move packages) — 程式碼組織的單位，包含模組 (modules)、依賴項 (dependencies) 和發布於 Sui 區塊鏈 (Sui blockchain) 上的地址 (addresses)。
title: 套件 (Package)
keywords:
  - Move
  - Sui
...
單一程式會被組織成一個套件。一個套件會被發布到區塊鏈上，並由一個[地址](./address)來識別。已發布的套件可以透過發送呼叫其函式的[交易](./what-is-a-transaction)來與之互動。它也可以作為其他套件的依賴項。
...
在本機上，一個套件是一個包含 `Move.toml` 檔案與 `sources` 目錄的資料夾。`Move.toml` 檔案——稱為「套件清單 (package manifest)」——包含套件的中繼資料，而 `sources` 目錄則包含模組的原始碼。一個套件通常看起來像這樣：
...
在開發期間，套件尚未擁有地址，此時會以 `0x0` 代替。一旦套件被發布後，它會在區塊鏈上獲得一個獨特的[地址](./address)，其中包含其模組的位元組碼 (bytecode)。已發布的套件會變成*不可變的 (immutable)*，並且可以透過發送交易來與其互動。
...
雖然已發布的位元組碼永遠無法被修改，但套件可以進行*升級 (upgraded)*：升級會在新的地址發布新版本的套件，同時保留舊版本。我們將在本書中探討相關的影響：[套件升級 (Package Upgrades)](./../programmability/package-upgrades)章節說明了其運作機制，而[可升級性實務做法 (Upgradeability Practices)](./../guides/upgradeability-practices)則涵蓋了如何為升級進行設計。
