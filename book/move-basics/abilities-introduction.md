...
沒有能力的結構既不能被捨棄、複製，也不能儲存在儲存空間中。我們將這種結構稱為 **熱馬鈴薯 (Hot Potato)**。這是一個輕鬆的名字，但它是記住沒有能力的結構就像「熱馬鈴薯」一樣，只能傳來傳去並且需要特殊處理的好方法。熱馬鈴薯是 Move 中最強大的模式之一，我們將在 [熱馬鈴薯模式](./../programmability/hot-potato-pattern) 章節中進行更詳細的介紹。
