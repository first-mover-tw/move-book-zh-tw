...
Macro 函式看起來、用起來都像一般函式，但它在執行期並不存在。相反地，編譯器會_展開_（expand）這個
...
- 它們的主體是在展開_之後_、依每個呼叫點分別做型別檢查，這使得一些一般[泛型](./generics)無法表達的操作
  成為可能——我們馬上就會看到。
...
> 另外請注意主體開頭的 `let a = $a;` 綁定。Macro 引數是以_運算式_的形式替換，而不是以計算過的值替換：
...
> 在呼叫點展開還有另一個可見的效果：macro 主體內觸發的 abort 會回報 macro _呼叫點_的行號，而不是
