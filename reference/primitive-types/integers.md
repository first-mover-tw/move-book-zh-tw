...
每種整數類型都支援相同的一組受檢查的算術操作。對於所有這些操作，兩個參數（左側和右側運算元）*必須* 具有相同的類型。如果您需要對不同類型的數值進行操作，則需要先執行 [轉型 (cast)](#轉型)。同樣地，如果您預期運算結果對於該整數類型來說太大，請在執行運算前先 [轉型](#轉型) 為更大的類型。
...
整數類型是 Move 中 *唯一* 可以使用比較運算子的類型。兩個參數都需要具有相同的類型。如果您需要比較不同類型的整數，您必須先 [轉型](#轉型) 其中之一。
...
與所有具備 [`drop`](./../abilities) 能力的類型一樣，所有整數類型都支援 ["相等"](./../equality) 和 ["不相等"](./../equality) 操作。兩個參數都需要具有相同的類型。如果您需要比較不同類型的整數，您必須先 [轉型](#轉型) 其中之一。
...
與語言中內建的其他純量值一樣，整數值是隱式可複製的，這意味著它們可以在沒有明確指令（如 [`copy`](./../variables#移動與複製)）的情況下被複製。
