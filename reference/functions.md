...
`public` 函式可以被定義在 _任何_ 模組中的 _任何_ 函式呼叫。如下例所示，`public` 函式可以被以下對象呼叫：
...
`public(package)` 可見性修飾符是 `public` 修飾符的一種更受限形式，旨在對函式的使用位置提供更多控制。`public(package)` 函式可以被以下對象呼叫：
...
在不修改 VM 源代碼的情况下，程式設計師無法添加新的原生函式。此外，`native` 函式的設計初衷是用於標準程式庫代碼或該 Move 環境所需的功能。

你可能看到的大多數 `native` 函式都在標準程式庫代碼中，例如 `vector`。
