...
巨集 (Macro) 函式是定義函式的一種方式，這些函式在編譯時會在每個呼叫點進行展開。巨集的引數不像普通函式那樣被提早求值 (evaluated eagerly)，而是透過表達式進行替換。此外，呼叫者可以透過 [Lambdas](#lambdas) 向巨集提供程式碼。

這種表達式替換機制使得 `macro` 函式類似於[在其他程式語言中發現的巨集](<https://en.wikipedia.org/wiki/Macro_(computer_science)>)；然而，在 Move 中它們受到的限制比你可能從其他語言預期的更多。`macro` 函式的參數和回傳值仍然是具備類型的 —— 儘管這可以透過 [`_` 類型](./../generics#_-類型) 來部分放寬。不過，這種限制的好處在於 `macro` 函式可以在任何普通函式可以使用的中央使用，這在 [方法語法 (Method Syntax)](./../method-syntax) 中特別有用。

未來可能會推出更廣泛的[語法巨集系統](<https://en.wikipedia.org/wiki/Macro_(computer_science)#Syntactic_macros>)。

## 語法

`macro` 函式的語法與普通函式類似。但是，所有類型參數名稱和所有參數名稱必須以 `$` 開頭。請注意，`_` 仍然可以單獨使用，但不能作為前綴，必須改用 `$_`。

```text
<可見性>? macro fun <識別碼><[$類型參數: 約束],*>([$參數名稱: 類型],*): <回傳類型> <函式主體>
```
...
這裡的 `$` 是用來指示參數（包括類型參數和數值參數）的行為不像它們非巨集的對應部分。對於類型參數，它們可以用任何類型實例化（甚至是引用類型 `&` 或 `&mut`），並且它們會滿足任何約束。同樣對於參數，它們不會被提早求值，相反，引數表達式將在每次使用時被替換。

## Lambdas

Lambdas 是一種新型表達式，只能與 `macro` 一起使用。它們用於將程式碼從呼叫者傳遞到 `macro` 的主體中。雖然替換是在編譯時完成的，但它們的使用方式類似於其他語言中的[匿名函式 (Anonymous functions)](https://en.wikipedia.org/wiki/Anonymous_function)、[lambdas](https://en.wikipedia.org/wiki/Lambda_calculus) 或 [閉包 (Closures)](<https://en.wikipedia.org/wiki/Closure_(computer_programming)>)。

如上例所示 (`$f: |$T| -> $U`)，lambda 類型的定義語法為：
...
Lambda 表達式隨後在 `macro` 的呼叫點使用以下語法定義：
...
請注意，如果標註了回傳類型，lambda 的主體必須封裝在 `{}` 中。
...
```move
let doubled: vector<u64> = map!(v, |x: u64| 2 * x); // 回傳類型標註可選
let bytes: vector<vector<u8>> = map!(v, |x: u64| -> vector<u8> { std::bcs::to_bytes(&x) });
```

### 捕捉 (Capturing)

Lambda 表達式還可以引用定義 lambda 的範圍內的變數。這有時被稱為「捕捉」。
...
與普通函式一樣，`macro` 函式是具備類型的 —— 參數和回傳值的類型必須經過標註。但是，函式的主體在巨集展開之前不會進行類型檢查。這意味著並非給定巨集的所有用法都是有效的。例如：
...
類型參數可以用任何類型實例化，包括引用類型 `&` 和 `&mut`。它們也可以用[元組類型](./../primitive-types/tuples)實例化，儘管目前這類操作的效用有限，因為元組無法綁定到變數。

這種放寬限制迫使類型參數的約束在呼叫點以一種通常不會發生的方式被滿足。然而，通常還是建議為類型參數添加所有必要的約束。例如：
...
只有當 `$T` 用具備 `copy` 能力的類型實例化時，此巨集才能展開。
...
對 `make_box` 的建議宣告是將 `copy` 約束添加到類型參數中。這樣就能告知呼叫者該類型必須具備 `copy` 能力。
...
那麼你可能會問，如果建議是不使用這種放寬，為什麼還要具備這種放寬呢？原因是類型參數上的約束在所有情況下都無法被強制執行，因為主體在展開之前是不會檢查的。在以下範例中，簽名中對 `$T` 的 `copy` 約束不是必需的，但在主體中卻是必需的。
...
然而，如果你想擁有一個極其寬鬆的類型簽名，建議改用 [`_` 類型](#_-類型)。

### `_` 類型

通常，[`_` 佔位符類型](./../generics#_-類型) 用於表達式中，以允許對類型引數進行部分標註。然而，在 `macro` 函式中，`_` 類型可以用來代替類型參數，以便為任何類型放寬簽名。這應該能增加宣告「泛型」`macro` 函式的便利性。
...
如果我們改用類型參數來宣告該函式，則類型必須統一為共同的類型，這在這種情況下是不可能的。
...
在這種情況下，`$T` 必須實例化為單一類型，但推斷發現 `$T` 必須同時綁定到 `u8` 和 `u16`。

不過這其中也存在權衡，因為 `_` 類型對於呼叫者而言傳達的意義和意圖較少。考慮將上面宣告的 `map` 巨集改用 `_` 代替 `$T` 和 `$U`。

```move
macro fun map($v: vector<_>, $f: |_| -> _): vector<_> {
```

在類型層級上不再有任何關於 `$f` 行為的指示。呼叫者必須從註釋或巨集主體中獲得理解。

## 展開與替換 (Expansion and Substitution)

`macro` 的主體在編譯時被替換到呼叫點。每個參數都被其引數的「表達式」而非「數值」所替換。對於 lambdas，可以在 `macro` 主體的上下文中綁定額外的本地變數數值。
...
再次強調，替換的不是 `x` 的值，而是表達式 `5`。這可能意味著一個引數會被求值多次，或者根本不求值，具體取決於 `macro` 的主體。
...
當滿足條件時，`return 'any true` 會提前退出「循環」。否則，該巨集將「回傳」`false`。
...
`macro` 函式的參數必須始終作為表達式使用。它們不能用於引數可能被重新解釋的情況。例如，以下情況是不允許的：
...
原因在於，如果引數 `$x` 不是引用，它會被先借用，這可能會重新解釋該引數。要繞過這項限制，你應該將引數綁定到本地變數。
...
此巨集計算除 `u256` 以外的任何整數類型的整數平方根。

`$T` 是輸入的類型，`$bitsize` 是該類型中的位元數，例如 `u8` 有 8 位元。`$U` 應設置為下一個較大的整數類型，例如 `u8` 對應 `u16`。

在此 `macro` 中，整數常值 `1` 和 `0` 的類型經過了標註，例如 `(1: $U)`，這允許常值的類型隨每次呼叫而不同。同樣地，`as` 可以與類型參數 `$T` 和 `$U` 一起使用。只有當 `$T` 和 `$U` 使用整數類型實例化時，此巨集才能成功展開。
...
Lambda 不一定非得在循環中使用，它們在條件式應用程式碼時通常非常有用。
