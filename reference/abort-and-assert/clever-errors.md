...
聰明錯誤是一項功能，可在斷言失敗或中止被觸發時提供更具資訊性的錯誤訊息。它們是一項原始碼功能，編譯成 `u64` 中止碼值，包含存取行號、常數名稱和常數值所需的資訊，並根據宣告聰明錯誤常數的模組而定。由於這個編譯，需要進行後處理才能從 `u64` 中止碼值轉換為人類可讀的錯誤訊息。Sui GraphQL 伺服器以及 Sui CLI 會自動執行後處理。如果您想手動解碼聰明中止碼，可以使用 [擴展聰明中止碼](#擴展聰明中止碼) 中概述的流程進行。
...
## 擴展聰明中止碼 (Inflating Clever Abort Codes)
