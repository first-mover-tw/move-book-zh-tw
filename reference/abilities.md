...
`store` 能力允許具有此能力的型別的數值存在於儲存中的某個數值內，但_不一定_是作為儲存中的頂層數值。這是唯一不直接限制操作的能力。相反，它與 `key` 配合使用時限制了在儲存中的存在。
...
如果 `Cup` 可以容納任何型別，無論其能力如何，這將非常有幫助。型別系統可以「看到」型別參數，因此如果它「看到」一個會違反該能力保證的型別參數，它應該能夠移除 `Cup` 的能力。
