...
對於 Sui 而言，`key` 被用來表示一項 [物件 (Object)](./abilities/object)。物件是存儲的基本單位，每個物件都有一個唯一的 32 位元組 ID。`store` 則用於指示哪些資料可以存儲在物件內部，同時也用於指示哪些類型可以被傳輸到其定義模組之外。
...
- [`store`](#store)
  ...
  - 對於 Sui 而言，`store` 控制哪些資料可以存儲在 [物件 (Object)](./abilities/object) 內部。`store` 還控制哪些類型可以被傳輸到其定義模組之外。
- [`key`](#key)
...
`copy` 能力允許具有該能力的類型的數值被複製。它限制了使用 [`copy`](./variables#move-and-copy) 運算子從本地變數複製數值的能力，以及透過[解引用 `*e`](./primitive-types/references#reading-and-writing-through-references) 透過引用複製數值的能力。
...
- `vector<T>` 可能具有 `copy`、`drop` 和 `store`，具體取決於 `T` 的能力。
  - 詳情請參閱[條件能力與泛型類型](#conditional-abilities-and-generic-types)。
