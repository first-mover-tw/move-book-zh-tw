...
泛型可用於在不同的輸入資料型別上定義函式和結構體。這項語言特性有時被稱為參數化多型 (parametric polymorphism)。在 Move 中，我們通常會將「泛型」與「型別參數 (type parameters)」及「型別引數 (type arguments)」這幾個術語互換使用。
...
更正式地說，如果一個型別被用作虛像型別參數的引數，我們稱該型別出現在「虛像位置 (phantom position)」。有了這個定義，正確使用虛像參數的規則可以描述為：**虛像型別參數僅能出現在虛像位置**。
